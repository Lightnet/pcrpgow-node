...
module.exports = function (routes, passport, config) {
    //console.log(config);
    // normal routes ==================================================
    ...
    */
    ...
    // =============================================================================
    // locally --------------------------------
    routes.get('/connect/local', function (req, res) {
        res.render('connect-local.ejs', { message: req.flash('loginMessage') });
    });
    routes.post('/connect/local', passport.authenticate('local-signup', {
        successRedirect: '/profile',
    ...
    // send to facebook to do the authentication
    if (config.benable_facebook) {
        routes.get('/connect/facebook', passport.authorize('facebook', { scope: 'email' }));
        // handle the callback after facebook has authorized the user
        routes.get('/connect/facebook/callback', passport.authorize('facebook', {
            //successRedirect : '/profile',
            successRedirect: '/',
            failureRedirect: '/'
        }));
    ...
    if (config.benable_twitter) {
        ...
        routes.get('/connect/twitter', passport.authorize('twitter'));
        // handle the callback after twitter has authorized the user
        routes.get('/connect/twitter/callback', passport.authorize('twitter', {
            //successRedirect : '/profile',
            successRedirect: '/',
            failureRedirect: '/'
        }));
    ...
    // send to google to do the authentication
    if (config.benable_google) {
        routes.get('/connect/google', passport.authorize('google', { scope: ['profile', 'email'] }));
        // the callback after google has authorized the user
        routes.get('/connect/google/callback', passport.authorize('google', {
            //successRedirect : '/profile',
            successRedirect: '/',
            failureRedirect: '/'
        }));
    ...
    // local -----------------------------------
    routes.get('/unlink/local', function (req, res) {
        var user = req.user;
    ...
    // facebook -------------------------------
    if (config.benable_facebook) {
        routes.get('/unlink/facebook', function (req, res) {
            var user = req.user;
    ...
    // twitter --------------------------------
    if (config.benable_twitter) {
        routes.get('/unlink/twitter', function (req, res) {
            var user = req.user;
    ...
    // google ---------------------------------
    if (config.benable_google) {
        routes.get('/unlink/google', function (req, res) {
            var user = req.user;
